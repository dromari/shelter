...
const showCards = (startNumber = 0) => {
  for (let i = 0; i < cards.length; i++) {
    ...
    let animalsNumber = (startNumber + i) % 8;
...
const getLeftPage = () => {
  let nextPage = getNumber();
  startNumber -= nextPage;
  if (startNumber < 0) startNumber = 8 + startNumber;
  showCards(startNumber);
}
...
leftPage.addEventListener('click', getRightPage);
rightPage.addEventListener('click', getLeftPage);
const items = shuffle();
showCards();